...
class LineButton: UIButton {
    ...
    @IBInspectable var leftCornerRadius: Double = 0.0 {
        didSet {
            let maskPAth1 = UIBezierPath(roundedRect: self.bounds,
                                         byRoundingCorners: [.topLeft , .bottomLeft],
                                         cornerRadii:CGSize(width:CGFloat(leftCornerRadius), height:CGFloat(leftCornerRadius)))
    ...
    @IBInspectable var rightCornerRadius: Double = 0.0 {
        didSet {
            let maskPAth1 = UIBezierPath(roundedRect: self.bounds,
                                         byRoundingCorners: [.topRight , .bottomRight],
                                         cornerRadii:CGSize(width:CGFloat(rightCornerRadius), height:CGFloat(rightCornerRadius)))
            let maskLayer1 = CAShapeLayer()
            maskLayer1.frame = self.bounds
            maskLayer1.path = maskPAth1.cgPath
            self.layer.mask = maskLayer1
